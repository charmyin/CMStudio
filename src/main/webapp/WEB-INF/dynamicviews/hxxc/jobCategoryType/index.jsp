...
<html>
    ...
    <script type="text/javascript">
      ...
      $(function(){
        ...
        //Load grid
        $("#dataGrid").datagrid({
            url:'hxxc/jobCategoryType/findAll',
            method:'POST',
            toolbar:'#toolbar',
            pagination:true,
            collapsible:true,
...
/********************************************************Initial the page end*****************************************************/
      ...
      //Open new form
      function newForm(){
          $('#dlg').dialog('open').dialog('setTitle','职位分类类别新增');
          $('#fm').form('clear');
          url = 'hxxc/jobCategoryType/save';
      }
      /**Open edit form **/
      function editForm(){
          ...
          if (rowsLength>0){
              $('#dlg').dialog('open').dialog('setTitle','职位分类类别修改');
              $('#fm').form('load',selectedRow[0]);
              url = 'hxxc/jobCategoryType/update';
          }else{
      ...
      function deleteSelectedItems(){
          ...
          if (rowsLength>0){
              $.messager.confirm('提示信息','确定删除该行？',function(r){
                  if (r){
                    $.post('hxxc/jobCategoryType/remove', {_id:rows[0]._id}, function(result){
                          if(result.success){
                              $.messager.show({
                                  title: '提示<span style="color:red;">!</span>',
